...
class SingleCombatDodgeMissileTask(SingleCombatTask):
    ...
    def get_obs(self, env, agent_id):
        """
        Convert simulation states into the format of observation_space将仿真状态转换为观测_空间的格式
    ...
    def reset(self, env):
        """Reset fighter blood & missile status重置战斗机和导弹状态
        """
    ...
    def step(self, env):#发射导弹
        ...
        for agent_id, agent in env.agents.items():
            ...
            if shoot_flag:
                ...
                env.add_temp_simulator(
                    MissileSimulator.create(parent=agent, target=agent.enemies[0], uid=new_missile_uid))#创建导弹指令、临时的（导弹发射不是一直的是有装弹过程），调用一个类MissileSimulator
                self.remaining_missiles[agent_id] -= 1#
