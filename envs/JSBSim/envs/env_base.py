...
from ..core.simulatior import  BaseSimulator,MissileSimulator,waitMissileSimulator #,AircraftSimulator
...
class BaseEnv(gym.Env):
    ...
    @property
    def agents(self) -> Dict[str, MissileSimulator]:# 原 Dict[str, AircraftSimulator]
        return self._jsbsims

    @property
    def time_interval(self) -> float: #int
    ...
    def load_simulator(self):
        ...
        self._jsbsims = {}  # type: Dict[str,MissileSimulator,waitMissileSimulator] #,waitMissileSimulator
        for uid, config in self.config.missile_configs.items():
            self._jsbsims[uid] =[MissileSimulator(
                uid=uid,
                color=config.get("team", "Red"),
                model=config.get("model", "AIM-9L"),
                init_state=config.get("init_state")
                # origin=getattr(self.config, 'battle_field_center', (120.0, 60.0, 0.0)),
                # sim_freq=self.sim_freq,
                # num_missiles=config.get("missile", 0)
            ),
                waitMissileSimulator(
                uid=uid,
                color=config.get("team","Blue"),
                model=config.get("model", "AIM-10L"),
                init_state=config.get("init_state"),
            #     # origin=getattr(self.config, 'battle_field_center', (120.0, 60.0, 0.0)),
            #     sim_freq=self.sim_freq,
            #     # num_missiles=config.get("missile", 0)
