...
class MissilePostureReward(BaseRewardFunction):
    ...
    def get_reward(self, task, env, agent_id):
        """
        Reward is velocity attenuation of the missile 导弹的速度衰减
        ...
        """
        reward = 0
        missile_sim = env.agents[agent_id].check_missile_warning()#定义与simulator：check_missile_warning 是否还剩有导弹 返回值 none、missile
        if missile_sim is not None:
            ...
            angle = np.dot(missile_v, aircraft_v) / (np.linalg.norm(missile_v) * np.linalg.norm(aircraft_v))#dot 矩阵乘法
            if angle < 0:
