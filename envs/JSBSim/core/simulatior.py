...
class BaseSimulator(ABC):
    ...
    def __init__(self, uid: str, color: TeamColors, dt: float):
        ...
        Args:
            ...
            UID（STR）：用于唯一标识的5位十六进制数字。
            Color（TeamColors）：使用不同的颜色字符串表示不同的团队
            DT（float）：模拟时间步长。默认值=`1/60`。
        """
        self.__uid = uid
        self.__color = color
        self.__dt = dt
...
class AircraftSimulator(BaseSimulator):
    ...
    def __init__(self,
        ...
        Args:
            ...
            参数:UID（STR）：用于唯一标识的5位十六进制数字。默认值=`“ A0100 ”`。
            Color（TeamColors）：使用不同的颜色字符串表示不同的团队型号
            模型（STR）：要装载的飞机名称。默认值=“ F16 ”。 模型路径： './data/aircraft_name/aircraft_name.xml'
            初始化_状态（DICT）：将属性映射到其初始值的字典。
            输入空字典以使用一组默认的初始属性。
            原点（元组）：全球战斗领域的原点（经度、纬度、海拔）。
            默认值=`（120.0，60.0，0.0）
            `SIM_频率（整数）：JSBSIM积分频率。默认值=`60`。
        """
        ...
        self.partners = []  # type: List[AircraftSimulator,waitMissileSimulator]#是不是应该改为字典加入敌机、我方战机Dict
        self.enemies = []   # type: List[AircraftSimulator,MissileSimulator]
        # temp simulator links
        self.launch_missiles = []   # type: List[MissileSimulator,waitMissileSimulator]
        self.under_missiles = []    # type: List[MissileSimulator,waitMissileSimulator]
    ...
    def run(self):
        ...
        JSBSim监视模拟并检测它是否认为它应该结束，例如因为指定了模拟时间。
        返回False如果满足JSBSIM终止条件。
...
class MissileSimulator(BaseSimulator): #来袭弹 攻击飞机
    ...
    def __init__(self,
        ...
        self._Isp = 60     # average specific impulse平均比冲 half
        self._Length = 2.87
        self._Diameter = 0.127
        self._cD = 0.4      # aerodynamic drag factor
        self._m0 = 84       # mass, unit: kg
        self._dm = 6        # mass loss rate, unit: kg/s
        self._K = 3         # proportionality constant of proportional navigation比例导航的比例常数
    ...
    @property
    def is_alive(self):
        """Missile is still flying发射"""
        return self.__status == MissileSimulator.LAUNCHED

    @property
    def is_success(self):
        """Missile has hit the target击中"""
        return self.__status == MissileSimulator.HIT

    @property
    def is_done(self):
        self.exploded = """Missile is already exploded"""
        return self.__status == MissileSimulator.HIT \
    ...
    @property
    def K(self):
        """Proportional Guidance Coefficient 比例导引系数"""
        # return self._K
        return max(self._K * (self._t_max - self._t) / self._t_max, 0)

    @property
    def S(self):
        """Cross-Sectional area, unit m^2  横截面积"""
    ...
    def launch(self, parent: AircraftSimulator):
        ...
        self._distance_pre = np.inf#没有确切的数值，为浮点型
        self._distance_increment = deque(maxlen=int(5 / self.dt))  # 5s of distance increment -- can't hit  5秒的距离增量--打不中    deque类似于list的容器，可以快速的在队列头部和尾部添加、删除元素
...
class waitMissileSimulator(BaseSimulator):#拦截弹 目标是来袭弹
    ...
    @classmethod
    def create(cls, parent: AircraftSimulator, target: MissileSimulator, uid: str, missile_model: str = "AIM-10L"):#目标来袭弹
        assert parent.dt == target.dt, "integration timestep must be same!"
        missile = waitMissileSimulator(uid, parent.color, missile_model, parent.dt)
        missile.launch(parent)
        missile.target(target)
        return missile
    ...
    def __init__(self,
                ...
                init_state=" ",
                 dt=1 / 12):
        super().__init__(uid, color, dt)
        self.__status = waitMissileSimulator.INACTIVE
        self.model = model
        self.parent_aircraft = None  # type: AircraftSimulator
        self.target_missile = None  # type: MissileSimulator
        self.render_explosion = False

        # missile parameters (for AIM-10L)拦截弹有优势
        self._g = 9.81      # gravitational acceleration
        self._t_max = 60    # time limitation of missile life
        self._t_thrust = 3  # time limitation of engine
        self._Isp = 120     # average specific impulse平均比冲
        self._Length = 2.87
        self._Diameter = 0.127
        self._cD = 0.4      # aerodynamic drag factor
        self._m0 = 84       # mass, unit: kg
    ...
    def is_alive(self):
        """Missile is still flying发射"""
        return self.__status == waitMissileSimulator.LAUNCHED

    @property
    def is_success(self):
        """Missile has hit the target击中"""
        return self.__status == waitMissileSimulator.HIT

    @property
    def is_done(self):
        self.exploded = """Missile is already exploded"""
        return self.__status == waitMissileSimulator.HIT \
            or self.__status == waitMissileSimulator.MISS
    ...
    def launch(self, parent: AircraftSimulator):
        ...
        self._m = self._m0
        self._dtheta, self._dphi = 0, 0
        self.__status = waitMissileSimulator.LAUNCHED
        self._distance_pre = np.inf#没有确切的数值，为浮点型
        self._distance_increment = deque(maxlen=int(5 / self.dt))  # 5s of distance increment -- can't hit  5秒的距离增量--打不中    deque类似于list的容器，可以快速的在队列头部和尾部添加、删除元素
        self._left_t = int(1 / self.dt)  # remove missile 1s after its destroying

    def target(self, target: MissileSimulator):
    ...
    def run(self):
        ...
        if distance < self._Rc and self.target_missile.is_alive:
            self.__status = waitMissileSimulator.HIT
            ...
            self.__status = waitMissileSimulator.MISS
        else:
            self._state_trans(action)
